...
export default function LoginVerify() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(false);
    if (!r.ok) { toast.error(r.error || "Invalid code"); return; }
  ...
  const resend = async () => {
    ...
    setBusy(false);
    if (!r.ok) { toast.error(r.error || "Could not resend"); return; }
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-7 text-foreground">
        ...
        <form onSubmit={submit} className="space-y-5">
          ...
          <div className="flex items-center justify-between text-xs">
            <button type="button" onClick={() => nav("/login")} className="text-muted-foreground hover:text-accent flex items-center gap-1">
